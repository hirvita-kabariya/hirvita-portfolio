...
const sd=async t=>{if(!t.trim()||ld)return;const u=t.trim();setIp("");setMs(p=>[...p,{role:"user",text:u}]);setLd(true);try{const h=[{role:"system",content:sys},...ms.map(m=>({role:m.role==="assistant"?"assistant":"user",content:m.text})),{role:"user",content:u}];const r=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer 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"},body:JSON.stringify({model:"llama-3.3-70b-versatile",max_tokens:1000,messages:h})});const d=await r.json();if(d.error){setMs(p=>[...p,{role:"assistant",text:"API Error: "+d.error.message}]);}else{setMs(p=>[...p,{role:"assistant",text:d.choices?.[0]?.message?.content||"Error."}]);}}catch(e){console.error(e);setMs(p=>[...p,{role:"assistant",text:"Connection error: "+e.message}]);}setLd(false);};
return(<div>
